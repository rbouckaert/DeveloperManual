calculate_ranks_draws_matrix_nonrandom <- function (params, dm)
{
  ...
  less_matrix <- sweep(dm,
                       MARGIN = 2,
                       STATS = params,
                       FUN = "<")
  rank_min <- colSums(less_matrix)
  equal_matrix <- sweep(dm,
                        MARGIN = 2,
                        STATS = params,
                        FUN = "==")
...
get_statistics <- function(index,
  ...
  if (ncol(postdraws) != K)
    stop("Parameters and draws don't match")
  ...
  if (rank_smooth) {
    ranks <-
      SBC:::calculate_ranks_draws_matrix(variables = parameters[-K],
                                         dm = fit_matrix)
  } else{
    ranks <-
      calculate_ranks_draws_matrix_nonrandom(params = parameters[-K],
                                             dm = fit_matrix)
  }
  ...
  return(dplyr::left_join(stats, HPDs))
}
