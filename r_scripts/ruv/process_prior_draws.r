...
lengths.prior <-
  unlist(parallel::mclapply(prior.draws, functional_1,
                            mc.cores = Ncores))
maxbranch.prior <-
  unlist(parallel::mclapply(prior.draws, functional_2,
                            mc.cores = Ncores))
rangebranch.prior <-
  unlist(parallel::mclapply(prior.draws, functional_2b,
                            mc.cores = Ncores))
...
if (do_distances) {
  RFs.prior <-  unlist(parallel::mclapply(prior.draws, functional_4,
                                          mc.cores = Ncores))
  BHVs.prior <-
    unlist(parallel::mclapply(prior.draws, functional_5,
                              mc.cores = Ncores))
