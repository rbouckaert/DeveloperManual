...
c.plot <- ggplot(sims[1:1000,], aes(x=C)) + geom_histogram(aes(y =..density..), bins=10, fill = "gray") + stat_function(fun = dnorm, args = list(mean = mean(sims[1:1000,]$B), sd = sd(sims[1:1000,]$C))) + xlim(-2.8,2.8) + xlab(expression("y"["C"])) + ylab("Density") + theme_classic() + theme(panel.background = element_rect(fill = "transparent"), plot.background = element_rect(fill = "transparent", color = NA))
...
pdf(file=paste0(output.path, "bmsim_rightpanel.pdf"), width=5, height=5)
