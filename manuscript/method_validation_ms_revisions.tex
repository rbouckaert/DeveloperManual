...
% $f(d) = \int_\boldsymbol{\theta} f(d|\boldsymbol{\theta})f(\boldsymbol{\theta})d\boldsymbol{\theta}$, the probability of the data, is also known as the marginal likelihood or the model evidence.
...
% $\boldsymbol{T}$ is a matrix whose elements are deterministically defined by tree $\phi$'s topology and branch lengths; see Fig. 1).
...
\textcolor{blue}{Interested readers will find another example of this validation procedure in the supplement, where}\textcolor{red}{\st{we illustrate this procedure for}} the (parametric) sampling distribution \textcolor{blue}{is that} underlying the phylogenetic Brownian motion model (``PhyloBM''; \citealp{fel73}).
