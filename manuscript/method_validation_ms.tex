...
\begin{abstract}
  ...
  implemented.
  Both goals are difficult to achieve for several reasons, such as the  multidisciplinary nature of method development, and a still embryonic literature on good software development and statistical practices aimed at professionals from disparate fields.
  Here we provide guidelines for the validation of probabilistic model implementations, focusing on Bayesian approaches.
  This manuscript summarizes good practices for assessing the correctness of simulation and inference procedures under a model, and is available in the traditionally static print version as well as in a reproducible and executable form.
\end{abstract}
...
\begin{figure}{l}{5.5cm}
  ...
  \caption{The graphical representation of a simp''le Bayesian
    phylogenetic model.
...
\begin{center}
  ...
  \begin{tabular}{ p{0.7in} | p{1.5in} | p{1in} | p{1.3in} }
    ...
    diversitree & Several birth-death models & R & \citealp{fitzjohn12}\\
...
%   BEAST 1, MrBayes, RevBayes, allow sampling a distribution using MCMC.
% \item
...
When $n=100$ and $\alpha = 0.95$, the confidence interval for the number of simulations containing the correct data-generating parameter is between $90$ and $99$ (Table \ref{tab:coverage2}).
...
\begin{tabular}{@{}cccc@{}}
\toprule
Credibility level (\%) & n (replicates) & lower & upper \\ \midrule
\multirow{3}{*}{90}    & 100            & 84    & 95    \\
                       & 200            & 171   & 188   \\
                       & 500            & 436   & 463   \\
\multirow{3}{*}{95}    & 100            & 90    & 99    \\
                       & 200            & 184   & 196   \\
                       & 500            & 465   & 484   \\
\multirow{3}{*}{99}    & 100            & 97    & 100   \\
                       & 200            & 195   & 200   \\
                       & 500            & 490   & 499   \\ \cmidrule(l){1-4} 
\end{tabular}
\end{center}
\caption{Under a correctly implemented model, the number
  of true simulated values that fall within their corresponding $\alpha\times 100$\%-HPDs (coverage) is binomially distributed with $n$ trials and probability of success $\alpha$.
  Here we show the 95\% probability intervals for the coverage under different credibility levels and numbers of replicates.
...
We expand on this point using a different and much simpler model \textcolor{red}{(Supplementary Figs. X)}.
...
\end{wrapfigure}
Adherence to this distribution can be investigated by constructing histograms \citep{Talts2018} as well as by looking at the empirical cumulative distribution function (ECDF) and their confidence bands \citep{Sailynoja2021}, see Figure \ref{fig:ruv_conceptual} below.

\begin{figure}
  \includegraphics[width=\linewidth]{figures/sbc_conceptual.pdf}
  \caption{Miscalibration patterns in Rank-uniformity validation (RUV).
  In panel A the replicate-averaged posterior (RAP), i.e. the posterior one gets over all simulated data sets, is overdispersed compared to the prior.
...
We conduct RUV on the three scenarios described in the previous section, which make use of the model depicted in Figure 1.
...
As expected, under scenario 1 our model implementation passes the RUV -- as indicated by histogram bars and ECDF values falling within their 95\% confidence intervals (Fig. \ref{fig:ruv_yule}).

Under scenario 2, again as expected, our method failed RUV, as shown by the middle plots Fig.~\ref{fig:ruv_yule}.
...
\begin{figure}
  \includegraphics[width=\linewidth]{figures/sbc_Yule_BM_r.pdf}
  \caption{Rank-uniformity validation (RUV) results for the Brownian motion rate, $r$.
  In A we show the rank histograms from 100 replicates, obtained after 10\% burnin and thinning down to get 200 draws from the posterior.
  For Scenario 1, no anomalies are detected, while for Scenarios 2 and 3 over and underestimation are present, respectively.
  Panel B shows the corresponding ECDF plots, which help magnify the problems when they are present (Scenarios 2 and 3).
   }
  \label{fig:ruv_yule}
\end{figure}

\textcolor{red}{When a model implementation fails RUV, it can do so in different ways, and some are more straightforward to interpret.
\textcolor{blue}{For example, if [@Luiz: fill out]}.
For a more thorough discussion, see \cite{Talts2018}.}

\textcolor{red}{RUV results from scenario 3 (Fig. Xc), \textcolor{blue}{specifically the histogram bars falling out of their 95\% confidence intervals, } would suggest that our model implementation is incorrect.
...
Studies have examined how these factors affect estimation accuracy and precision (e.g., \citealp{zhang23}[cite Landis Anolis paper, Ho papers]), as well as the mixing and convergence of MCMC chains (e.g., \citealp{nylander04,zhang23}).
...
% \begin{itemize}
%     \item 
%     \item RUV failing for a single rank does not mean the method is flawed
% \end{itemize}
...
We further highlighted the difference between experiments in model validation versus model characterization.
...
% This manuscript is also followed by a supplementary text further illustrating these guidelines that is linked with a live document available on \href{https://github.com/rbouckaert/DeveloperManual}{https://github.com/rbouckaert/DeveloperManual}.
...
\subsection*{Funding}
F.K.M. was supported by and Marsden grant 16-UOA-277.
R.B. was supported by Marsden grant .
L.M.C was supported by ...
