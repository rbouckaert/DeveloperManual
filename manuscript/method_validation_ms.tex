...
$y_0$ is the trait value at the root of the species tree, $r$ is the variance of the process
...
\label{fig:1}
\captionof{figure}{A sample of 150 draws from a MVN distribution, each
  representing the evolutionary trajectory of one continuous trait along
  the species tree on the left. The root trait value, $y_0$, and the
...
Because the MVN is a \textbf{known parametric
  distribution}, it is somewhat trivial to verify the correctness of a
...
For a small tree (Fig. 1, left panel) the likelihood of a given set
of parameter values $\theta$ can be computed manually and compared to
...
Again using the Yule model as an example, the probability of observing
