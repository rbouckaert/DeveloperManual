...
\fancyhead[C]{Mendes et al. $\bullet$ June 2023 $\bullet$ bio{\color{red}R}$\chi$ve} % Custom header text
...
\begin{figure}
  \label{fig:sbctest}
  \includestandalone[width=12cm]{../figures/coverage_interval_plot}
\end{figure}
...
In the previous sections, we described and executed two procedures for validating Bayesian models, namely, coverage validation and simulation-based calibration (SBC).
...
This is because such protocols provide clear, objective rules for assessing model correctness, based on the coverage and rank distribution of parameters values; both be computed for any and all Bayesian models.
...
In contrast, there is an infinite number of ways in which a new or published method can have its behavior inspected.
Researchers may want to know, given a model, how sensitive parameter estimates are to prior choice, data set size, model assumption violation, to name a few.
The immense variety of such investigations and their idiosyncrasies should be self-evident.
We collectively refer to these examinations as ``model characterization'': any analysis of model behavior beyond assessing its correctness (as described in the present work).
Model characterization is no doubt critical for the longevity and popularity of a method, after all, researchers need to know if and when they can trust the output of an analysis.
