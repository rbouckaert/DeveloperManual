...
\begin{abstract}
  ...
  These tools should be fast and must be correct.
  But while the concept of speed is relatively straightforward, what passes as evidence for correctness varies widely among methodological contributions.
...
Scenario 3 brings home the point that an incorrect model implementation may pass coverage validation, unless model misspecification is sufficiently severe (e.g., scenario 2), or parameter estimate locations are highly responsive to the evidence in the data (unlike $\Lambda$ in the model at hand).
We expand on this point using a different and simpler model, and show that even coverage validation detects stringent rejection schemes as a model misspecification issue; Supplementary Table 2.
