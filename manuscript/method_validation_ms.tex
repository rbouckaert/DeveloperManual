...
Branching processes are stochastic processes that can organically capture the most fundamental tenet in evolutionary biology, namely common descent, at multiple time scales.
Over the last 15 years or so, the simplest examples of this class of processes have been generalized to allow for lineage-affecting events that vary over time, and that are state-dependent -- with state here meaning the attributes of a lineage's genotypic, phenotypic, ecological or geographic character(s).
Perhaps unsurprisingly, in this window of time branching processes (a.k.a. tree models) became key ingredients in the statistical treatment of evolutionary phenomena such as species diversification and infectious disease spread.

Although convenient evolutionary abstractions, tree models can be difficult to formalize because of their parameter space, which includes both a combinatorily complicated discrete component (the tree topology) and a continuous components (the branch lengths) \citep{semple03}.
The analysis of tree space is an active area of research in mathematical biology \citep{gavryushkin16} that sits outside the scope of this article.
We will nonetheless devote this section to synthesizing and commenting on different procedures used in the past to validate tree models specifically.

FKM's ideas:

\begin{itemize}
\item PG1/2: Simplest models (Yule, birth-death): theoretical expectations; TODO: plot root age and number of tips sampling from the prior (put in supplement);
...
\item PG1/2: Looking at theoretical expectations for gene tree composition \citep{rosenberg02} and coverage of tree models like the MSC \citep{ogilvie22}; not a branching process, but one in which tree structures play a role;
