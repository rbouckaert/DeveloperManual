...
\begin{figure}%{l}{5.5cm}
  ...
  \caption{
    ...
    A random variable within a rectangular box signifies a parameter whose value is assumed known by the user; these are normally nuisance hyperparameters, or parameters estimated elsewhere (e.g., the phylogeny in a comparative analysis) that are not of immediate interest.
...
\begin{enumerate}
  ...
  \item $\text{S}[f_{\Phi|\Lambda}(\cdot|\mathcal{O}=o,\Lambda=\lambda)]$, which simulates a Yule tree $\phi$ given a value of $o$ (the origin age) and $\lambda$ (the birth-rate) simulated in (1),
...
\begin{center}
  ...
  \begin{tabular}{ p{1in} p{1in} p{2in} p{2in} }
    ...
    \rowcolor{gray!10}Yule & Pure-birth & $f_{\Theta |\mathcal{O},\Lambda}(\cdot |\mathcal{O}\mathord{=}o,\Lambda\mathord{=}\lambda)$ & $f_{\Theta |\Lambda}(\lambda|\mathcal{O}\mathord{=}o,\Lambda\mathord{=}\lambda)$\\
...
Confirming that this is the case indicates $\text{S}[f_{\Phi|\Lambda}(\cdot|\mathcal{O}=o,\Lambda=\lambda)]$ is correctly implemented (Fig. \ref{fig:yulemean}).
...
\begin{figure}
  ...
  \includegraphics[width=14cm]{../figures/flowchart_manual2.pdf}
  \caption{Flowchart of the validation of a Bayesian model.
    ...
    Analogously, $\boldsymbol{d}$ denotes a vector with $n$ elements, where each element is an i.i.d. data sample from the corresponding likelihood(s) $f_{D|\Theta}(\cdot)$.
...
\begin{figure}
  ...
  \caption{
    ...
    (a): parameter estimates are overdispersed relative to their true values.
    (b): parameter estimates are underdispersed relative to their true values.
    (c): parameter estimates are consistently overestimated relative to their true values.
    (d): correctly specified model and inference machinery.
