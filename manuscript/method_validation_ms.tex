...
Lastly, in ``Scenario 3'' of figure \ref{fig:yulecalval} (rightmost top and bottom panels), we again correctly specified the model, but carried out substantial rejection sampling in simulation.
...
\textcolor{red}{We conduct SBC on the three scenarios described in the previous section, which revolve around the model depicted in figure 1.
...
As expected, under scenario 1 our model passes the SBC correctness test -- as indicated by histogram bars and ecdf values falling within their 95\% confidence intervals (Fig. Xa,c).
The outcome of SBC under scenario 2, again as expected, was a fail, as shown by \textcolor{blue}{[describe histogram and ecdf plots here, I can't remember what they look like by memory]}.
...
When a model implementation fails SBC, it can do so in different ways.
For a more thorough discussion, see \cite{Talts2018}.}

SBC results from scenario 1 indicate that the implementation of the
model in figure 1 is correct , the 
