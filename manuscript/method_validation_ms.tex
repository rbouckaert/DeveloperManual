...
The pragmatic individual with a real data set to analyze will likely find comfort in this line of reasoning.
...
If simulations must be conducted within a substantially constrained parameter space so as to yield realistic data -- or data whose probabilities can be calculated -- this may be a sign that the model needs to be modified. 
Independent implementations that also do not pass validation tests provide further evidence that the issue may be the model itself.
Historically, model design has often gone in the direction of incrementally conditioning the statistical process so as to match empirical observations (e.g., [add citations here]), but re-imagining the model entirely might be the best solution [ideas for citations?].}
