...
sampled from $I(M)$ with MCMC can be compared to a target distribution
($\text{P}(\theta)$) sampled with $S(M)$ (see box 2).
$S(M)$ can also be itself conveniently used by $I(M)$ as the proposal
density during MCMC (i.e., $S(M)$ generates $\theta$; see listing 2 in
the supplementary material for an example),
which then allows isolating the validation task to the model.
If other proposal densities are used, mismatched distributions can be
...
This second, expected distribution can be $\text{P}(\theta)$ or
obtained through MCMC with a mutually exclusive set of correctly
implemented operators (see item (a) in supplementary table S1).
