...
\item the model correctly estimates parameter values from data sets
  simulated from parameter priors, as indicated by the estimated posterior
  coverage of parameters and their correlation with true values
  (``well-calibrated validation'', see below). {\color{red}{If we want to talk about the next level -- using
...
involves simulating data sets under specific parameter
...
It is for this reason that validating a Bayesian model by trying to recover
hand-picked parameter values can be problematic. 
Here, it is unclear which prior is to be used, and also how often one should
...
prior distributions), but we instead recommend validations that are exact,
such as well-calibrated studies (see below).
