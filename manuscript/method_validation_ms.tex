...
We collectively refer to these examinations as ``model characterization'': any analysis of model performance during inference, beyond assessing its correctness (as described in the present work).
Although it may happen (e.g., \citealp{steel20}), model characterization is rarely carried out to satisfy the theoretical curiosity of a researcher; it is instead normally motivated by a model's empirical applications.
