...
One way of looking at this involves noting that, if $\phi$ and $\phi_0$ have very many ($s$) tips, they will share a number of clades that is approximately Poisson distributed with rate $c_{\phi}$, the number of cherries in $\phi$ \citep{bryant09}.
Given that the expected number of cherries in a random Yule tree asymptotes to $\frac{s}{3}$ \citep{mckenzie00}, one can see that in the case of Yule trees, $\phi$ and $\phi_0$ have approximately a 85$\%$ chance of not sharing any clades.
