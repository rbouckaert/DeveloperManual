...
\begin{tcolorbox}[breakable, width=\textwidth, colback=gray!10, boxrule=0pt, title=Box 1: Validating a phylogenetic model with respect to its phylogenetic tree parameter, fonttitle=\bfseries]
  \small 
  ...
  One such metric is the well-known Robinson-Foulds distance (RF; \citealp{Robinson1981}), which measures \textcolor{red}{[@Luiz: brief description of RF]} relative to another phylogenetic tree.
  ...
  Fig. 9 shows validation results for the RF metric for five-taxon trees simulated under a simple Kingman's coalescent model assuming a known effective population size.

  \begin{center}
    \includegraphics[width=0.92\linewidth]{../figures/coverage_ruv_coal_RF.pdf}
    \label{fig:rf}
    \captionof{figure}{
      Coverage validation and rank-uniformity validation (RUV) of a Kingman's coalescent model with respect to the Robinson-Foulds (RF) between the coalescent tree and another (random) tree.
      The effective population size parameter is assumed known.
      (a) The true RF distances (i.e., between the simulated coalescent trees and the same random tree; a total of 100 i.i.d. values) plotted against the corresponding mean posterior RF distances (calculated from posterior samples of the coalescent trees and the same random tree).
      ...
      Simulations for which 95\%-HPDs contained the true value are highlighted in blue, otherwise are presented in red.
      (b) RUV histograms of 100 ranks obtained after \textcolor{red}{X\%} burnin and thinning of posterior samples down to \textcolor{red}{X} out of \textcolor{red}{X}.
      (c) RUV empirical CDF plots.
    }
  \end{center}

  Fig. 9a shows the coverage of 95\%-HPD intervals of the RF metric, while Fig. 9b-c give its rank distribution and empirical CDF, respectively.
  The coverage of the RF statistic is very close to 95, and the rank distribution is approximately uniform on (1, $L+1$); together, these panels indicate this model is correctly implemented.
  In the supplement, we consider other phylogenetic tree metrics that could be used as an alternative or in addition to the RF distance (Supplementary Fig. \textcolor{red}{[@Luiz: put reference to supplementary figure here]}).
\end{tcolorbox}
...
Those with real data set to analyze will likely find pragmatic comfort in this line of reasoning.
