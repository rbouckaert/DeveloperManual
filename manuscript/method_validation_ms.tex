...
\fancyhead[C]{Mendes et al. $\bullet$ September 2023} %$\bullet$ bio{\color{red}R}$\chi$ve} % Custom header text
...
Tree models are stochastic processes that can organically capture the most fundamental tenet in evolutionary biology, namely common descent, at multiple time scales.
...
Given the interest in tree models shown by empirical, computational and theoretical biologists, in this section we will cover how tree models have been commonly validated in the past, with an emphasis on tree space.
...
Method developers should expect their software to often fail validation, especially at earlier development stages.
...
Our words are meant as encouragement to method developers: in our experience, the validation procedure is almost always arduous and repetitive, but very effective in revealing issues and later in giving modelers peace of mind when releasing their software.

As we showed above, however, a model inference machinery can still fail a validation test despite being correctly implemented in code.
This can happen when there is model misspecification as a result of users filtering out simulations that do not resemble the types of data and processes observed in the real world.
...
When the observed coverage is distant from its expectation or if RUV reveals over- or underestimated parameters, for example, a researcher may want to hypothesize about why validation is failing, and test those hypotheses with further analysis of simulated data.
If validation success is contingent upon simulation conditions that are empirically unrealistic or computationally intractable (e.g., trees with zero or an immense number of extant tips must be included), regularities emerging from further simulation experiments can in the very least illuminate what the causative agent is (see section 3 in the supplement for an example).
Developers may want to determine if not only their own, but also if existing independent implementations seem to approach correctness as model specification between inference and inference is improved.
Results replicated across independent methods could suggest an issue with the model as opposed to with the code underlying the implementation being validated.

The type of experimentation described above should be further useful for elucidating model behavior (see below) and uncovering potential biases caused by model misspecification.
We expect that the different extents to which an RUV procedure, in particular, can be made to fail (and how) will help method developers better characterize and report biases in parameter estimates [cite Sean's pre-print/paper?].
...
Here, post-processing and plotting MCMC results (e.g., Fig. \ref{fig:yulecalval}) can at least suggest parameter over- and underestimation biases.

Should passing validation procedures appear hopeless, and if the outcome of additional experiments is not enlightening, the method developer may understandably resort to downplaying the importance of the validation effort.
If ``all models are wrong'' when analyzing real data, and if the necessary conditions for successful validation are unrealistic, then maybe validating a model is not all that important.
This is an argument that will likely speak to the pragmatic individual.
But while a tempting thought, we urge researchers to consider how this perspective runs roughshod over one of the most fundamental procedure in empirical analyses: statistical hypothesis testing.
The objections above may be valid, but it stands to reason that the less evidence there is for one or more models being incorrectly implemented, the more skeptical one should be when drawing conclusions from empirical model comparison.
[Ultimately, one should still strive for confirming a model is correctly implemented.]
