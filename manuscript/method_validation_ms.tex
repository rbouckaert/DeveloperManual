...
evidence.
Crucially, note that a Bayesian model includes prior $\text{P}(\theta)$:
...
When MCMC is conducted, one can use the fact that $\text{P}(D)$
evaluates to a constant that can be conveniently ignored, i.e., $\text{P}(\theta|D) \propto \text{P}(\theta|D)\text{P}(\theta)$.

Assuming MCMC is the chosen technique for obtaining $\text{P}(\theta|D)$
...
\begin{enumerate}[i.]
  \item the model appears to produce reasonable results (perhaps more reasonable than other models)
    on a data set of interest,
