...
\textcolor{blue}{Tree models are stochastic processes that can capture the most fundamental tenet in evolutionary biology, namely common descent, at multiple time scales.
...
\textcolor{blue}{Although convenient evolutionary abstractions, tree models can nonetheless be challenging to formalize depending on their level of realism.
...
\textcolor{blue}{Given the interest in tree models shown by empirical, computational and theoretical biologists, in this section we will cover how tree models have been and can be validated, with an emphasis on tree space.
We also propose a new manner by which tree models can undergo RUV and be assessed with respect to coverage.
...
\textcolor{blue}{One way a tree model implementation can be validated is by comparing statistical summaries of its samples (drawn through direct simulation or MCMC without data) against theoretical ``target'' values (e.g., Fig. \ref{fig:yulemean}).
...
\textcolor{blue}{As tree models increase in complexity (e.g., \citealp{maddison07,goldberg12,fitzjohn10,scire22}), so does the tree space they define and as a consequence theoretical model validation as described above becomes difficult.
...
\textcolor{blue}{When theoretical predictions useful for validation cannot be made, an often-employed method involves the comparison of independent model implementations, with one or both being simulators or inference engines.
...
\textcolor{blue}{The procedures of coverage validation and RUV can also be used to examine parameters in tree space.
...
\textcolor{blue}{To the best of our knowledge, the RUV procedure has not been applied to validate phylogenetic models vis-a-vis tree space.
...
In what follows we fill this gap by introducing one possible solution for ranking trees as a pre-requisite for RUV.}

\textcolor{blue}{Briefly, we propose that each of the trees in a set of MCMC samples, as well as the corresponding ``true'' tree, be first compared to an external, ``reference'' tree (see Algorithm \ref{alg:sbc} in the supplementary material).
...
Method developers should expect their software to often fail validation, especially at early development stages
\textcolor{red}{\st{. New model implementations visit}}\textcolor{blue}{, causing} the loops in Fig. \ref{fig:flowchart} \textcolor{blue}{to be visited} many times\textcolor{red}{\st{ -- it is then that the most glaring, and later the more insidious, backend coding mistakes and model misspecification errors are uncovered and fixed.}}
\textcolor{red}{\st{Our words are meant as encouragement: in our experience, }}The validation procedure is almost always arduous and repetitive, but very effective in revealing issues and \textcolor{red}{\st{later}} in giving modelers peace of mind when releasing their software.
\textcolor{red}{\st{As we showed above, however, a model}}\textcolor{blue}{A correctly implemented} inference machinery can \textcolor{blue}{nonetheless} still fail a validation test\textcolor{red}{\st{ despite being correctly implemented in code.}} \textcolor{blue}{if there is some unforeseen form of}
\textcolor{red}{\st{This can happen when there is}} model misspecification \textcolor{blue}{(e.g., rejection sampling).}\textcolor{red}{\st{as a result of users filtering out simulations that do not generate the types of data sets observed in the real world.
If upon such failure researchers are confident that all coding mistakes have been ironed out, a}}\textcolor{blue}{In such cases, a potentially} delicate stage of method development begins, when \textcolor{red}{\st{further analysis can take place,}} decisions must be made \textcolor{blue}{between further testing or software release.}\textcolor{red}{\st{, and different resolutions are possible.}}

\textcolor{red}{\st{When the observed coverage is distant from its expectation or if RUV reveals over- or underestimated parameters, for example, a researcher may want to hypothesize about why validation is failing, and test those hypotheses with further analysis of simulated data.}}
If validation success is marginal or contingent upon \textcolor{blue}{a substantially constrained parameter space, or if a Bayesian method has good coverage
but fails the demanding RUV (as shown here and elsewhere; \citealp{mchugh22})}\textcolor{red}{\st{simulation conditions that are empirically unrealistic or computationally intractable (e.g., trees with zero or an immense number of extant tips must be included), regularities emerging from}} further simulation experiments might illuminate the nature of the model misspecification \textcolor{blue}{and suggest ways to modify the model}.
For example, developers may want to\textcolor{red}{\st{progressively }}tweak an aspect of simulation, and then \textcolor{blue}{repeat RUV in search for regularities in parameter over- or underestimation}\textcolor{red}{\st{check if their implementation seems to approach correctness}} (e.g., section 3 in the supplement).
\textcolor{blue}{When releasing a method despite validation failure,} researchers should in the very least be expected to report all attempts made to validate an implementation, why they seemed to fail, and what biases were uncovered, if any.
Ideally, guidelines should be provided for interpreting results obtained with a tool \textcolor{blue}{in the presence of any known biases.}

\textcolor{red}{\st{The type of experimentation described above should be further useful to those interested in characterizing model behavior (see below), even after it is published.
    We expect that the different extents to which an RUV procedure, in particular, can be made to fail (and how) will help method developers identify biases in parameter estimates (McHugh et al., 2022).
...
Here, graphical summaries of post-processed MCMC results (e.g., Fig. 3) can help uncover parameter estimate biases.}}
% \ref{fig:yulecalval}
% \citep{mchugh22}.

\textcolor{red}{\st{Lessons about models can thus be learned by attempting to validate a model implementation even if ultimately it cannot be shown to be correct.
...
\textcolor{red}{\st{Should passing validation procedures appear hopeless, the method developer may understandably find themselves downplaying the importance of the validation effort.
...
But while a tempting thought, we urge researchers to consider how this perspective runs roughshod over statistical hypothesis testing, a ubiquitous practice in empirical studies.
...
When confronted with utter validation failure, \textcolor{blue}{we urge} method developers to resist the temptation of downplaying the importance of the validation effort, and instead ask the hard question of whether their models are reasonable in the first place.
If large numbers of simulations must be rejected\textcolor{red}{\st{conducted within a substantially constrained parameter space}} so as to \textcolor{red}{\st{yield}}\textcolor{blue}{obtain} realistic data -- or data whose probabilities can be calculated -- this could be a sign that the model needs to be modified. 
...
Historically, model design has often gone in the direction of incrementally conditioning the statistical process so as to match empirical observations (e.g., \citealp{Gelman1995,Gelman2020}), but re-imagining the model entirely might be the best solution.
