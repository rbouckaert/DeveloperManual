...
%DIF DEL method_validation_ms.tex             Wed Jan 26 09:35:43 2022
%DIF ADD method_validation_ms_revisions.tex   Tue Feb  8 08:27:08 2022
...
%DIF -------
\author{\textsc{F\'{a}bio K. Mendes\DIFdelbegin \DIFdel{$^{1*}$}\DIFdelend \DIFaddbegin \DIFadd{$^{1\dagger*}$}\DIFaddend \RIGHTBRACE, \textsc{Remco Bouckaert\DIFdelbegin \DIFdel{$^{2*}$}\DIFdelend \DIFaddbegin \DIFadd{$^{2\dagger*}$}\DIFaddend \RIGHTBRACE,\\
\textsc{\DIFdelbegin \DIFdel{Christiaan Swanepoel$^{2}$}\DIFdelend \DIFaddbegin \DIFadd{Luiz M. Carvalho$^{3\dagger}$}\DIFaddend \RIGHTBRACE, \textsc{Alexei J. Drummond\DIFdelbegin \DIFdel{$^{1,2}$}\DIFdelend \DIFaddbegin \DIFadd{$^{4}$}\DIFaddend \RIGHTBRACE \\
...
\small
\DIFdelbegin %DIFDELCMD < \href{mailto:f.mendes@auckland.ac.nz}{Corresponding authors$^*$:
%DIFDELCMD <   f.mendes@auckland.ac.nz, remco@cs.auckland.ac.nz}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \href{mailto:f.mendes@auckland.ac.nz}{$^*$Corresponding authors:
...
called the Metropolis-Hastings \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{metropolis53,mh} }\hspace{0pt}%DIFAUXCMD
algorithm }\DIFdelend \DIFaddbegin \DIFadd{algorithm \mbox{%DIFAUXCMD
\citep{metropolis53,mh}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend , only requires the
posterior to be evaluated up to a constant.
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend In practice, the Metropolis-Hastings algorithm samples the posterior
...
\DIFdelbegin \DIFdel{We will spend time considering MCMC in particular as it is the commonly
...
}\DIFdelend %DIF >  We will spend time considering MCMC in particular as it is the commonly chosen technique for obtaining $f(\theta|d)$ under an implementation of model $\mathcal{M}$.
%DIF >  A thorough validation effort thus entails verifying the correctness of (i) the model (i.e., $f(d|\theta)f(\theta)$), and (ii) the components involved in the MCMC transition mechanism.
We note that the \DIFdelbegin \DIFdel{latter are }\DIFdelend \DIFaddbegin \DIFadd{transition mechanism is }\DIFaddend not part of the model, however\DIFdelbegin \DIFdel{, and it is possible to sample $f(\theta|D)$ }\DIFdelend \DIFaddbegin \DIFadd{.
It is possible in fact to sample $f(\theta|d)$ }\DIFaddend with other techniques such as
...
%DIFDELCMD < %%%
\DIFdel{Finally, we stress that we are interested in }\DIFdelend \DIFaddbegin \DIFadd{Below, we focus on }\DIFaddend practices for verifying model \emph{correctness} \DIFdelbegin \DIFdel{.
There are other tests employed to ensure that a particular }\DIFdelend \DIFaddbegin \DIFadd{--
...
machinery.
Tests to ensure an }\DIFaddend MCMC analysis is converging as anticipated
\DIFdelbegin \DIFdel{.
...
}\DIFdelend \DIFaddbegin \DIFadd{exist, but are outside the scope of this work.
%DIF >  Determining that one or more independent Markov chains converged on very similar posterior distributions is not a correctness test, as those distributions might be very different from the target distribution.
}\DIFaddend 
...
(ii) \DIFdelbegin \DIFdel{$S[f(\Phi|\lambda)]$}\DIFdelend \DIFaddbegin \DIFadd{$\text{S}[f(\Phi|\lambda)]$}\DIFaddend , which simulates a Yule tree $\Phi$ \DIFdelbegin \DIFdel{, (iii)$S[f(\mathbf{A}|\Phi,r)]$, which simulates a multiple sequence
alignment $\mathbf{A}$}\DIFdelend \DIFaddbegin \DIFadd{given a
value of $\lambda$ simulated in (i)}\DIFaddend , and (\DIFdelbegin \DIFdel{iv)
$S[f(\mathbf{M}|\Phi,r_m,y_0])$}\DIFdelend \DIFaddbegin \DIFadd{iii)
$\text{S}[f(\boldsymbol{y}|\Phi,r)]$}\DIFaddend , which simulates \DIFdelbegin \DIFdel{a continuous character vector $\mathbf{M}$ for all
...
counterparts (i.e.
, the values used as input in the simulation), or to their analytical expectations.
...
\begin{center}
\DIFdelbegin %DIFDELCMD < \includegraphics[width=11cm]{../figures/bmsim.pdf}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \includegraphics[width=11cm]{../figures/bmsim.png}
\DIFaddend \label{fig:bmsim}
...
Figure 1 shows the result of 2000 simulations under the MVN
with $\boldsymbol{y_0} = [0.0, 0.0, 0.0]$, $r = 0.1$, along }\DIFdelend \DIFaddbegin \DIFadd{well-characterized parametric distribution.
...
The latter comes from the variance-covariance matrix; for }\DIFaddend the
tree shown \DIFdelbegin \DIFdel{on
the left, which determines the variance-covariance matrix
:
}\DIFdelend \DIFaddbegin \DIFadd{in figure 1 %DIF >  \ref{fig:bmsim}
...
\DIFadd{The more complex the natural phenomenon under study,
...
\DIFadd{Deciding on the utility of a model for real-world problems is a
...
\end{wrapfigure}
\DIFadd{Analyses of model fit against data are normally accompanied by discussions on
...
\begin{figure}
  ...
  \caption{\DIFdelbeginFL \DIFdelFL{Calibrated validation }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{\textcolor{red}{[This figure is being updated]}
    ...
    hierarchical model \DIFdelbeginFL \DIFdelFL{. }\DIFdelendFL (\DIFdelbeginFL \DIFdelFL{a) The graphical representation of the model used in
    the analyses, where $\lambda$ is the Yule birth-rate, $n$ is the
    number of species, and $\tau$ is the set of speciation times}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Fig}\DIFaddendFL . \DIFdelbeginFL \DIFdelFL{The
    prior is an exponential density with arbitrary mean of 0.01}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{\ref{fig:pgm})}\DIFaddendFL . Panels \DIFdelbeginFL \DIFdelFL{(b-d)
    ...
    with $50<n<200$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Dots and lines represent true values and their 95\%-HPDs,
    respectively.
    Simulations for which 95\%-HPDs contained the true value are
    highlighted in blue}\DIFaddendFL , \DIFaddbeginFL \DIFaddFL{otherwise are presented in red.
...
\DIFadd{We illustrate the SBC procedure on the model represented in figure
