...
% \lstset{language=XML}
\section{Coverage validation using the Experimenter package for BEAST2: a worked example}

\indent In this document we present a worked example of a coverage validation study using BEAST 2's Experimenter package.
...
The hierarchical model whose implementation we will validate in this document is less pedagogical than the one covered in the main manuscript, but more closely resembles a typical model employed by empirical phylogeneticists.
...
Inspecting the `truth.log` with the Tracer app reveals that the simulator works as expected (the means and variances of parameters match those specified in the XML file).
Note that we generated 101 samples ($n=100$ if we follow the main manuscript's notation; with an extra simulation for executing RUV in tree space), but nothing prevents more data sets to be generated.
...
At this point, we can run the \texttt{CoverageCalculator} tool (part of the Experimenter package) from the Terminal to create coverage summary statistics:
...
To do a RUV analysis, we must first combine the log files produced during inference into a single file:
...
The Terminal output is shown in supplementary figure \ref{supfig:terminal2}; it displays the histograms as well as ECDF plots (not all of them shown due to size of screen display).
...
\begin{figure}
  ...
  \caption{Frequency parameter 2 has one bin that is outside the expected range (indicated by the empty bin).}
