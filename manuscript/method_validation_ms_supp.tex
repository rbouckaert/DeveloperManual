...
\textcolor{magenta}{To formally assess the correctess of the clade frequency diagnostic in Figure~\ref{supfig:clade_covg} one might do one of two things: (i) check that the resulting empirical cumulative distribution function (ECDF) fits that of an appropriately scaled uniform distribution using a Kolmogorov-Smirnov test and/or (ii) fit a regression model to the attained frequencies against the bin midpoints and test the null hypothesis $H_0: \text{Intercept}=0\: \text{and}\: \text{Slope}=1$ and reject at the usual confidence levels.} 
