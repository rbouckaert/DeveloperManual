...
\small $^3$Escola de Matem\'{a}tica Aplicada, Fundaç\~{a}o Get\'{u}lio Vargas\\
...
\begin{table}[h]
  ...
  \begin{tabular}{ c|c|c }
    ...
    $\text{Var}[y_s]$ & A & 93\\
              & B & 97\\
              & C & 97\\
    $\text{Cov}[y_s,y_v]$ & A and B & \textcolor{red}{[@Luiz: fill out]}\\
              & A and C & \textcolor{red}{[@Luiz: fill out]}\\
              & B and C & \textcolor{red}{[@Luiz: fill out]}\\
...
\begin{table}[h]
  ...
  \begin{tabular}{ c|c|c }
    ...
    Metric & Notation & Ref. \\
    \hline  
    The largest branch length in \st{$\tau$} $\Phi$ & $\text{LB}(\Phi)$ & N/A\\
    The length of $\Phi$ (the sum of all branch lengths \st{$S(\tau)$}) & $\text{LEN}(\Phi)$ & N/A\\
    The length of the external branch leading to taxon $s_1$ & $\text{T}_1(\Phi)$ & N/A\\
    The Robinson-Foulds distance between \st{$\tau$} $\Phi$ and \st{$\tau_0$} $\Phi_0$ & $\text{RF}_0(\Phi)$ & \citep{Robinson1981}\\
    The Kendall-Coljin distance between \st{$\tau$} $\Phi$ and \st{$\tau_0$} $\Phi_0$ & $\text{KC}_0(\Phi;\lambda)$ & \citep{Kendall2016}\\
    The Billera-Holmes-Vogtman distance between \st{$\tau$} $\Phi$ and \st{$\tau_0$} $\Phi_0$ & $\text{BHV}_0(\Phi)$ & \citep{Billera2001}\\
    \hline
  \end{tabular}
\end{table}
...
\begin{algorithm}[h]
  ...
  \caption{Algorithm for carrying out a simulation-based validation procedure with respect to the phylogenetic tree parameter $\Phi$.}\label{alg:sbcphylo}
  ...
  \For{$i \gets 1$ \KwTo $n$} {
    ...
    $\boldsymbol{\theta}_i^j \gets$ \MCMC{$f(\theta|d_i)$}\; \Comment*[r]{$\boldsymbol{\theta}_i^j = \{\boldsymbol{\theta}_i^1, \boldsymbol{\theta}_i^2..., \boldsymbol{\theta}_i^L\}$, where $L$ is the number of MCMC samples}
    $\bar{\delta}_i \gets$ \Distance{$\phi_0, \phi_i$}\; \Comment*[r]{According to a distance metric of choice}
    $\boldsymbol{\delta}_i^j \gets$ \Distance{$\phi_0, \phi_i^j$}\; \Comment*[r]{$\boldsymbol{\delta}_i^j = \{\delta_i^1, \delta_i^2, ..., \delta_i^L\}$}
...
\textcolor{red}{[@Luiz: could you please fix the notation in the
  proof? My notation appears here in the supplementary, but also in
  the main text]}
For a number \st{$M$} $n$ of simulations, simulate
...
\noindent where $Q_{y}(x)$ is the posterior CDF and $p_1, p_2 \in (0,1)$, $p_1 < p_2$.
...
\noindent where the last line follows from the fact that the CDF of $\theta^{(i)}$ is uniformly distributed on $(0, 1$) (Theorem 1 in \cite{Cook2006}) and almost sure convergence of the ECDF to the true CDF (Glivenko-Cantelli theorem).
