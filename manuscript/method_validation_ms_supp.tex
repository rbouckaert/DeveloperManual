...
\begin{itemize}
 \item The largest branch length in $\Phi$, $\text{LB}(\Phi)$;
 \item The length of the phylogeny, i.e.,  the sum of branch lengths $\text{LEN}(\Phi)$;
 \item The length of the external branch leading to taxon $s_1$, $\text{T}_1(\Phi)$;
 \item The Robinson-Foulds \citep{Robinson1981} distance between $\Phi$ and $\Phi_0$, \st{$\operatorname{RF}_0(\Phi)$} $\text{RF}_0(\Phi)$;
 \item The Kendall-Coljin \citep{Kendall2016} distance between $\Phi$ and $\Phi_0$, \st{$\operatorname{KC}_0(\Phi;\lambda)$} $\text{KC}_0(\Phi;\lambda)$; 
 \item The Billera-Holmes-Vogtman (BHV) \citep{Billera2001} distance between $\Phi$ and $\Phi_0$, $\text{BHV}_0(\Phi)$;
\end{itemize}
...
\end{algorithm}
  ...
  \item Generate a reference tree from the prior $\bar{\Phi}_0  \sim \pi_T(\Phi | \boldsymbol \gamma)$; 

    \textbf{for} each iteration in 1:N, \textbf{do}:
    
  \item Generate $\bar{\Phi} \sim \pi_T(\Phi | \boldsymbol \gamma)$;
  \item Compute the distance $\bar{\delta} = d_\sigma(\bar{\Phi},\bar{\Phi}_0)$ according to the metric of choice;
  \item Generate some (aligment) data $\tilde{y} \sim f(y | \bar{\Phi}, \boldsymbol\alpha)$;
  \item Draw (approximately) $\boldsymbol \Phi_s = \{\Phi_s^{(1)}, \Phi_s^{(2)}, \ldots, \Phi_s^{(L)}\}$ from the posterior $\pi(\Phi | \tilde{y})$;
