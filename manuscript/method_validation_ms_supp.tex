...
\begin{algorithm}[h]
  ...
  $\phi_0 \gets$ \SampleTree{$\theta_\Phi$}\; \Comment*[r]{$\phi_0 \sim f_T(\Phi|\theta_\Phi)$}
  \For{$i \gets 1$ \KwTo $n$} {
    ...
    $\phi_i \gets$ \SampleTree{$\theta_{i\Phi}$}\; \Comment*[r]{$\phi_i \sim f_{T\mid \Theta}(\phi|\theta_{i\Phi})$}
    $d_i \gets$ \SampleData{$\phi_i,\boldsymbol{\theta}_i^r$}\; \Comment*[r]{$d_i \sim f_{D\mid \Phi}(d|\phi_i,\boldsymbol{\theta}_i^r$}
