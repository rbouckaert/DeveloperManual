...
In the main text, we also mentioned the possibility of assessing the coverage of a phylogenetic tree's topology by looking at statistics of its clades.
For example, to formally assess the correctess of the clade frequency diagnostic (Supplementary Fig.~\ref{supfig:clade_covg}) one might do one of two things.
First, one can check that the resulting empirical cumulative distribution function (ECDF) fits that of an appropriately scaled uniform distribution using a Kolmogorov-Smirnov test.
